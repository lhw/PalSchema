...
namespace PS {
    ...
    RC::StringType SpawnerInfo::ToString()
    {
        ...
        RC::StringType location = fmt::format(TEXT("X: {:.3f}, Y: {:.3f}, Z: {:.3f}"), Location.GetX(), Location.GetY(), Location.GetZ());

        if (Type == SpawnerType::Sheet)
        {
            CachedString = fmt::format(TEXT("(Sheet @ [{}] with {} group{})"),
                location,
                SpawnGroupList.size(),
        ...
        CachedString = fmt::format(TEXT("({} @ [{}])"), npcId, location);;
        return CachedString;
    }
